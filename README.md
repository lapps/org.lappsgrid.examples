...
For example, our tokenizer service might __require__ plain text with no annotations and then __produce__ a sequence of tokens in LIF format. Thus, 
...
Service vendors can generate metadata JSON at compile time or runtime. We will go through two different ways to use Lappsgrid modules to generate metadata in this tutorial. However developers are always allowed to generate this string their own way.

## Option #1. using `Metadata` module

One way to generate a JSON object is to use the [`metadata`](http://lapps.github.io/org.lappsgrid.metadata/groovydoc) module. It contains wrapping classes for generating JSON strings:
...
(Pay special attention when the service is not a maven project: `version` should be specified in java annotation or in `VERSION` file; or it won't compile.)
...
As the last step, we need a method in the `WhitespaceTokenizer` class to read the JSON file generated by compilation.
